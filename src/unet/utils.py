...
class ModelWrapper:
    ...
    def measure_large_images(self, dataset, visualize_bboxes=False, filter=True, export_path=None,
        ...
        for batch_idx, (X_batch, image_filename) in enumerate(DataLoader(dataset, batch_size=1)):
            ...
            bboxes = []
            for region in hypo_result:
                minr,minc,maxr,maxc = region.bbox
                bboxes.append((minr,minc,maxr,maxc))
            
            all_bboxes[image_filename[0]] = bboxes
            
            #save all bounding boxes to a file
            bboxes_file = os.path.join(export_path, f'{image_filename[0][:-4]}_bboxes.npy')
            np.save(bboxes_file,bboxes)
