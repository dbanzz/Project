...
def make_mask(hypo_mask_path, nonhypo_mask_path, export_path=None):
    hypo = io.imread(hypo_mask_path)
    nonhypo = io.imread(nonhypo_mask_path)
    
    # Ensure masks are binary (assuming 255 for mask and 0 for background)
    hypo_mask = (hypo == 255).astype(np.uint8) * 2  # Set to 2 for hypo regions
    nonhypo_mask = (nonhypo == 255).astype(np.uint8) * 1  # Set to 1 for nonhypo regions

    # Check for overlap: if there is overlap, resolve by prioritizing hypo regions
    overlap = hypo_mask & nonhypo_mask
    if np.any(overlap):
        nonhypo_mask[overlap > 0] = 0  # Remove nonhypo in overlapping areas
    
    # Combine masks: nonhypo regions are overwritten by hypo where overlap was resolved
    combined_mask = np.maximum(hypo_mask, nonhypo_mask)

    if export_path:
        io.imsave(export_path, combined_mask)
    else:
